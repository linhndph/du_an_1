...
include "../models/trangthaica.php";
...
if (isset($_GET['act']) && ($_GET['act'] != "")) {
    ...
    switch ($act) {
        ...
        case 'themdondatsan':
            if (isset($_POST['booknow']) && ($_POST['booknow'])) {
                    ...
                    if (isset($_SESSION["dondatsan"])){
                        ...
                        header("location:ClientController.php?act=themdondatsan");
                    }
                    else{
                        $_SESSION["dondatsan"] = [];
                        $id_san = $_POST['idsan'];
        ...
        case 'thanhtoan':
            ...
            if (isset($_SESSION["ten_dang_nhap"])) {
                ...
                if (isset($_POST['thanhtoan']) && ($_POST['thanhtoan'])) {
                    ...
                    $ca_san = $_POST['ca_san'];
                    $trang_thaihd="Chờ xử lý";
                    $ma_dat_san = "#PolyScoccer" . rand(0, 99999);
                    $ngay_su_dung=$_POST['ngay_su_dung'];
                    $hd = insert_hoa_don($ma_dat_san, $tong_don_hang,$id_tk ,$ten_kh, $sdt_kh, $email, $dia_chi_kh,$ten_san,$ca_san,$trang_thaihd,$ngay_su_dung);
                    // $id_TaiKhoan = loadone_tk($_SESSION['id_tai_khoan']);
            ...
            include "../views/Client/thongtintt.php";
            break;

        case 'xoadsdatsan':
            $billsan = loadone_billdatsan($_SESSION['id_tai_khoan']);
                if (isset($_GET['id_ctdatsan']) && $_GET['id_ctdatsan']){
                    $id_ctdatsan=$_GET['id_ctdatsan'];
                   if(count(delete_datsanhd123($id_ctdatsan))==0){
                   
                    header('location: ?act=xemhoadon&err=true');
                    die();
                    
                   }else{
                    delete_datsan($id_ctdatsan);
                    $id_ca=$_GET["id_ca"];
                    $id_san=$_GET["id_san"];
                    $id_trang_thai=1;
                    update_trangthaica($id_san,$id_ca,$id_trang_thai);
                   }
                }
               header('location: ?act=xemhoadon');
                break;
        ...
        case 'xemhoadon':
            ...
            include "../views/Client/hoadondatsan.php";
            break;
